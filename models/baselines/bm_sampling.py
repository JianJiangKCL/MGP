...
class BiasMimicDatasetPreprocessor(nn.Module):
	...
	def preprocess_dataset_(self, train_ds, modalities, attribute, target_personality):
		...
		self.targets_bin = y_train_copy

		# stack all modalities in this modality combination
		...
		S_train = S_train_dict[attribute].numpy().astype(float)
		self.sensitive_targets = S_train

		self.set_dro_info()
		self.bias_mimick()
		self.print_new_distro()
		train_ds.init_bias_mimic(self.targets_bin)
